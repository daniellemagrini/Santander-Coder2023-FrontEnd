import { chinelosBD } from "./chinelo";
import { adicionarAoCarrinho } from "./menuCarrinho";

export function renderizarCatalogo() {
  for (const produtoCatalogo of chinelosBD) {
    const cartaoProduto = `<div class='border-solid w-48 m-2 flex flex-col p-2 justify-between shadow-xl shadow-slate-400 rounded-lg group ${
      produtoCatalogo.infantil ? "infantil" : "adulto"
    }' id="card-produto-${produtoCatalogo.id}">
        <img
        src="../assets/img/${produtoCatalogo.imagem}"
        alt="${produtoCatalogo.descricao}"
        class='group-hover:scale-110 duration-300 my-3 rounded-lg'
        />
        <p class='text-sm'>${produtoCatalogo.descricao}</p>
        <p class='text-sm py-2'>R$ ${produtoCatalogo.preco.toFixed(2)}</p>
        <button id='adicionar-${
          produtoCatalogo.id
        }' class='bg-slate-950 hover:bg-slate-700 text-slate-200'
        ><i class="fa-solid fa-cart-plus"></i></button>
        </div>`;

    document.getElementById("container-produto").innerHTML += cartaoProduto;
  }

  for (const produtoCatalogo of chinelosBD) {
    document
      .getElementById(`adicionar-${produtoCatalogo.id}`)
      .addEventListener("click", () => adicionarAoCarrinho(produtoCatalogo.id));
  }
}
